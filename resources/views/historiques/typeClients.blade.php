            ...
            <table class="table table-customize m-0">
              ...
              @foreach ($typeClients as $typeClient)
                  <tr>
                    ...
                    <td class="align-middle">
                     {{ date("d/m/Y",strtotime($typeClient->created_at)) }}
                    </td>
                    <td class="align-middle">
                     {{ date("H:i:s",strtotime($typeClient->created_at)) }}
                    </td>
                    <td class="align-middle">
                      <button type="button" class="btn btn-primary waves-effect waves-light p-0 px-1" data-bs-toggle="modal" data-bs-target="#display{{ $typeClient->id }}">
                        <span class="mdi mdi-eye-outline"></span>
                      </button>
                      <div class="modal fade" tabindex="-1" id="display{{ $typeClient->id }}" aria-labelledby="mySmallModalLabel" style="display: none;" aria-hidden="true">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body p-2">
                                    @php
                                          ...
                                          $array_sup = [];
                                          if($typeClient->event == "created" ){
                                              foreach($typeClient->properties['attributes'] ?? [] as $attribute => $value){
                                                if(!is_null($value)){
                                                  $array_insert[] = [
                                                    "champ"=>$attribute,
                                                    "value"=>$value,
                                          ...
                                          elseif ($typeClient->event == "deleted") {
                                            foreach($typeClient->properties['old'] ?? [] as $attribute => $value){
                                              if(!is_null($value)){
                                                $array_sup[] = [
                                                  "champ"=>$attribute,
                                                  "value"=>$value,
                                    ...
                                    @endphp
                                    @php
                                      $properties = json_decode($typeClient->properties, true);
                                      $changedAttributes = [];
                                    ...
                                    @if (count($changedAttributes) > 0)
                                      ...
                                      @endforeach
                                    @elseif($typeClient->event == "created")
