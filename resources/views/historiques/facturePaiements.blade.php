            ...
            <table class="table table-customize m-0">
              ...
              @foreach ($facturePaiements as $facturePaiement)
                  <tr>
                    ...
                    <td class="align-middle">
                     {{ $facturePaiement->user }}
                    </td>
                    <td class="align-middle">
                     {{ date("d/m/Y",strtotime($facturePaiement->created_at)) }}
                    </td>
                    <td class="align-middle">
                     {{ date("H:i:s",strtotime($facturePaiement->created_at)) }}
                    </td>
                    <td class="align-middle">
                      <button type="button" class="btn btn-primary waves-effect waves-light p-0 px-1" data-bs-toggle="modal" data-bs-target="#display{{ $facturePaiement->id }}">
                        <span class="mdi mdi-eye-outline"></span>
                      </button>
                      <div class="modal fade" tabindex="-1" id="display{{ $facturePaiement->id }}" aria-labelledby="mySmallModalLabel" style="display: none;" aria-hidden="true">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body p-2">
                                    @php
                                          ...
                                          $array_sup = [];
                                          if($facturePaiement->event == "created" ){
                                              foreach($facturePaiement->properties['attributes'] ?? [] as $attribute => $value){
                                                if(!is_null($value)){
                                                  $array_insert[] = [
                                                    "champ"=>$attribute,
                                                    "value"=>$value,
                                          ...
                                          elseif ($facturePaiement->event == "deleted") {
                                            foreach($facturePaiement->properties['old'] ?? [] as $attribute => $value){
                                              if(!is_null($value)){
                                                $array_sup[] = [
                                                  "champ"=>$attribute,
                                                  "value"=>$value,
                                    ...
                                    @endphp
                                    @php
                                      $properties = json_decode($facturePaiement->properties, true);
                                      $changedAttributes = [];
                                    ...
                                    @if (count($changedAttributes) > 0)
                                      ...
                                      @endforeach
                                    @elseif($facturePaiement->event == "created")
                                    <h6 class="text-center title">
                                      {{ $facturePaiement->event }}
                                    </h6>
                                      <ul class="list-group">
