            ...
            <table class="table table-customize m-0">
              ...
              @foreach ($achatCheques as $achatCheque)
                  <tr>
                    ...
                    <td class="align-middle">
                     {{ date("d/m/Y",strtotime($achatCheque->created_at)) }}
                    </td>
                    <td class="align-middle">
                     {{ date("H:i:s",strtotime($achatCheque->created_at)) }}
                    </td>
                    <td class="align-middle">
                      <button type="button" class="btn btn-primary waves-effect waves-light p-0 px-1" data-bs-toggle="modal" data-bs-target="#display{{ $achatCheque->id }}">
                        <span class="mdi mdi-eye-outline"></span>
                      </button>
                      <div class="modal fade" tabindex="-1" id="display{{ $achatCheque->id }}" aria-labelledby="mySmallModalLabel" style="display: none;" aria-hidden="true">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body p-2">
                                    @php
                                          ...
                                          $array_sup = [];
                                          if($achatCheque->event == "created" ){
                                              foreach($achatCheque->properties['attributes'] ?? [] as $attribute => $value){
                                                if(!is_null($value)){
                                                  $array_insert[] = [
                                                    "champ"=>$attribute,
                                                    "value"=>$value,
                                          ...
                                          elseif ($achatCheque->event == "deleted") {
                                            foreach($achatCheque->properties['old'] ?? [] as $attribute => $value){
                                              if(!is_null($value)){
                                                $array_sup[] = [
                                                  "champ"=>$attribute,
                                                  "value"=>$value,
                                    ...
                                    @endphp
                                    @php
                                      $properties = json_decode($achatCheque->properties, true);
                                      $changedAttributes = [];
                                    ...
                                    @if (count($changedAttributes) > 0)
                                      ...
                                      @endforeach
                                    @elseif($achatCheque->event == "created")
